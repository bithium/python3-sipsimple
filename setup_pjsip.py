...
if sys_platform == "darwin":
    ...
    local_cflags = " %s %s %s %s %s -mmacosx-version-min=%s -isysroot %s" % (
        arch_flags,
        ossl_cflags,
        sqlite_cflags,
        opus_cflags,
    ...
    local_ldflags = " %s %s %s %s %s -isysroot %s" % (
        arch_flags,
        ossl_ldflags,
        sqlite_ldflags,
        opus_ldflags,
...
class build_ext(cython_build_ext):
    config_site = [
        ...
        "#define PJMEDIA_AUDIO_DEV_HAS_COREAUDIO %d"
        % (1 if sys_platform == "darwin" else 0),
        ...
        "#define PJMEDIA_HAS_WEBRTC_AEC %d"
        % (1 if re.match(r"i\d86|x86|x86_64", platform.machine()) else 0),
        ...
        "#define PJMEDIA_VIDEO_DEV_HAS_DSHOW %d"
        % (1 if sys_platform == "win32" else 0),
    ...
    def distutils_exec_process(cmdline, silent=True, input=None, **kwargs):
        ...
        if returncode != 0:
            raise RuntimeError(
                ...
                % (returncode, " ".join(cmdline), stderr.decode())
            )
    ...
    def get_makefile_variables(cls, makefile, silent=True):
        ...
        stdout = cls.distutils_exec_process(
            [cls.get_make_cmd(), "-f", makefile, "-pR", makefile], silent=silent
        )
        return dict(
            tup
            for tup in re.findall("(^[a-zA-Z]\w+)\s*:?=\s*(.*)$", stdout, re.MULTILINE)
        )
    ...
    def configure_pjsip(self, silent=True):
        ...
        if ffmpeg_path is not None:
            cmd.append(
                "--with-ffmpeg=%s" % os.path.abspath(os.path.expanduser(ffmpeg_path))
            )

        libvpx_path = env.get("SIPSIMPLE_LIBVPX_PATH", None)
        if libvpx_path is not None:
            cmd.append(
                "--with-vpx=%s" % os.path.abspath(os.path.expanduser(libvpx_path))
            )
        ...
        if amr_nb_path is not None:
            cmd.append(
                "--with-opencore-amr=%s"
                % os.path.abspath(os.path.expanduser(amr_nb_path))
            )

        amr_wb_path = env.get("SIPSIMPLE_AMR_WB_PATH", None)
        if amr_wb_path is not None:
            cmd.append(
                "--with-opencore-amrwbenc=%s"
                % os.path.abspath(os.path.expanduser(amr_wb_path))
            )
        ...
        self.distutils_exec_process(
            cmd, silent=not self.verbose, cwd=self.build_dir, env=env
        )
        if (
            "#define PJ_HAS_SSL_SOCK 1\n"
            not in open(
                os.path.join(
                    self.build_dir, "pjlib", "include", "pj", "compat", "os_auto.h"
                )
            ).readlines()
        ):
            os.remove(os.path.join(self.build_dir, "build.mak"))
            raise DistutilsError(
                "PJSIP TLS support was disabled, OpenSSL development files probably not present on this system"
            )
    ...
    def update_extension(self, extension, silent=True):
        build_mak_vars = self.get_makefile_variables(
            os.path.join(self.build_dir, "build.mak")
        )
        ...
        extension.libraries = self.get_opts_from_string(
            build_mak_vars["PJ_LDLIBS"], "-l"
        )
        ...
        if sys_platform == "darwin":
            ...
            frameworks = re.findall(
                "-framework (\S+)(?:\s|$)", build_mak_vars["PJ_LDLIBS"]
            )
            extension.extra_link_args = list(
                itertools.chain(*(("-framework", val) for val in frameworks))
            )
